...
dynamic e25Intrinsics(List<dynamic> args) {
  ...
  //   line 267: 'ey'
  final combinedResult = aot.invoke('String.+', <dynamic>[arg0, 'ey']);
  return false;
  // 5 branch region(s) and 0 loop(s) reconstructed; exact machine structure remains in reports/functions.json.
}

/// Partially reconstructed from package:edge_probe/probe_code.dart near line 254.
dynamic e24Knot(List<dynamic> args) {
  if (!(arg0)) {  /* medium-confidence predicate */
    if ((arg2 & (1 << 63)) == 0) {
      while ((arg2 >> 1) > 4) {
      }
    } else {
    ...
    if (!(arg1)) {  /* medium-confidence predicate */
      return 2;
    } else {
      return 1;
    }
...
dynamic e23DynamicApply(List<dynamic> args) {
  ...
  //   .toString(...) at 1 site(s): 77 candidate implementation(s), e.g. Abi.toString, ArgumentError.toString, AssertionError.toString, AsyncError.toString.
  // 1 branch region(s) and 0 loop(s) reconstructed; exact machine structure remains in reports/functions.json.
...
dynamic e17JsonRoundTrip(List<dynamic> args) {
  ...
  if ((jsonDecodeResult & 1) == 0) {  /* low-confidence predicate */
    if (x4 <= 2) {  /* low-confidence predicate */
      final mapResult =
        aot.invoke('Iterable.map', <dynamic>[aot.snapshotRef(55), aot.snapshotRef(17976), jsonDecodeResult, aot.unresolvedValue('shared-code result')]);
...
dynamic e16SortedCopy(List<dynamic> args) {
  final splayTreeSetResult = aot.invoke('dart:collection.SplayTreeSet', <dynamic>[aot.snapshotRef(18544), arg0]);
  ...
  final list = _List._of(aot.snapshotRef(18544), splayTreeSetResult);
  return list;
...
/// Partially reconstructed from package:edge_probe/probe_code.dart near line 127.
/// Inlined by the optimizer (statements live inside this body):
///   _GrowableList._literal2
///   _GrowableList._literal7
...
dynamic e10AsyncLoop(List<dynamic> args) async {
  while (x2 < arg0) {
    final futureResult = aot.invoke('dart:async._Future', <dynamic>[aot.snapshotRef(18530)]);
    if (x0 != x16) {  /* low-confidence predicate */
      final asyncCompleteResult =
        aot.invoke('_Future._asyncComplete', <dynamic>[futureResult, local20, futureResult, local20]);
      final combinedResult =
        aot.invoke('_IntegerImplementation.+', <dynamic>[aot.unresolvedValue('shared-code result'), local18, local18, aot.unresolvedValue('shared-code result')]);
  ...
  final future = Future.delayed(aot.snapshotRef(18499), const Duration());
...
dynamic e09TryRethrow(List<dynamic> args) {
  ...
  if ((local70 & 1) == 0) {  /* low-confidence predicate */
    if (x16 > 1) {  /* low-confidence predicate */
      ...
      while (true) {
        final combinedResult2 = aot.invoke('String.+', <dynamic>[local68, aot.snapshotRef(109)]);
        final combinedResult3 = aot.invoke('String.+', <dynamic>[local68]);
      }
    } else {
      final combinedResult4 = aot.invoke('String.+', <dynamic>[combinedResult3, aot.snapshotRef(109)]);
      return 'ok:${local70}';
    }
  } else {
  }
...
Map<String, num> e07GenericBound<T extends num, U extends T>(Map<String, U> arg0) {
  ...
  MapEntry<String, num> closureAtLine70(String arg0, U arg1) {
    if ((arg1 & 1) == 0) {
      final mapEntryResult = aot.invoke('dart:core.MapEntry', <dynamic>[aot.snapshotRef(18388)]);
      return mapEntryResult;
    } else {
    }
  }
  final mapResult =
    map(aot.unresolvedValue('shared-code result'), aot.snapshotRef(34655), aot.snapshotRef(18388), arg0, aot.unresolvedValue('shared-code result'));
  return mapResult;
    ...
    if (local10 != null) {  /* low-confidence predicate */
      if (x2 == null) {  /* low-confidence predicate */
        if (local10 != null) {  /* low-confidence predicate */
          ...
          final joinResult =
            aot.invoke('List.join', <dynamic>[aot.unresolvedValue('shared-code result'), aot.snapshotRef(34522), aot.snapshotRef(272)]);
        }
      } else {
...
dynamic e02Cascade(List<dynamic> args) {
  ...
  final sortResult = aot.invoke('List.sort', <dynamic>[growableList, aot.snapshotRef(34574)]);
...
dynamic e01InterpChain(List<dynamic> args) {
  ...
  final toStringAsFixedResult =
    aot.invoke('_Double.toStringAsFixed', <dynamic>[1, aot.unresolvedValue('shared-code result'), arg0, arg1]);
  return 'user=${arg0} id=${aot.unresolvedValue('interpolated part')}${aot.unresolvedValue('interpolated part')}${toStringAsFixedResult}${aot.unresolvedValue('interpolated part')}${aot.unresolvedValue('interpolated part')} bool=${arg1 > 10} nullish=${null}';
  // Control-flow evidence: 9 conditional branch(es), 2 loop back-edge(s), 0 exception handler(s).
  // Remaining block structure is preserved in reports/functions.json and reports/assembly.s.
}
...
abstract class E21Mode extends _Enum {
  ...
  static dynamic parse(List<dynamic> args) {
    final enumByName_byNameResult =
      aot.invoke('EnumByName|byName', <dynamic>[aot.snapshotRef(54), aot.snapshotRef(18106), arg0]);
