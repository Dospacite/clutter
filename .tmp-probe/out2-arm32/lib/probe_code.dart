...
int e19Ackermann(int arg0, [int arg1 /* default unavailable */]) {
  if (aot.unresolvedRegister('r1') < 2) {  /* low-confidence predicate */
    if (aot.unresolvedRegister('r3') != 0) {  /* low-confidence predicate */
      if (aot.unresolvedRegister('r6') != 0) {  /* low-confidence predicate */
        if (aot.unresolvedRegister('r4') == aot.unresolvedRegister('r0')) {  /* low-confidence predicate */
          final e19AckermannResult = e19Ackermann();
          if (aot.unresolvedValue('slot 0x4') == (aot.unresolvedValue('slot 0x4') << 1)) {  /* low-confidence predicate */
...
/// Partially reconstructed `e19Ackermann`.
final e19Ackermann_tearOff = e19Ackermann; // implicit closure forwarding to `e19Ackermann`
...
dynamic e25Intrinsics(List<dynamic> args) {
  ...
  if (aot.unresolvedRegister('r3') != 0) {  /* low-confidence predicate */
    ...
    if ((startsWithResult & 8) == 0) {  /* low-confidence predicate */
      if (0 >= aot.unresolvedValue('slot 0x8')) {  /* low-confidence predicate */
      } else {
        if (aot.unresolvedRegister('r1') != 188) {  /* low-confidence predicate */
          // unresolved predicate: r1 != 107 /* low-confidence predicate */
        } else {
          // unresolved predicate: r1 != 107 /* low-confidence predicate */
        }
      }
...
dynamic e24Knot(List<dynamic> args) {
  if (arg0 != aot.unresolvedRegister('r4')) {  /* low-confidence predicate */
    if (aot.unresolvedRegister('r8') < 0) {  /* low-confidence predicate */
      ...
      if (arg2 >= 0) {  /* low-confidence predicate */
        // Loop shape recovered without a provable predicate /* low-confidence predicate */.
        while (true) {
          if (aot.unresolvedRegister('r0') > 0) {  /* low-confidence predicate */
          }
          // unresolved predicate: r1 <= 4 /* low-confidence predicate */
        }
      }
    }
  } else {
    if (arg1 != aot.unresolvedRegister('r4')) {  /* low-confidence predicate */
      return 2;
    } else {
      return 1;
    }
...
dynamic e17JsonRoundTrip(List<dynamic> args) {
  ...
  if (aot.unresolvedRegister('r4') <= 2) {  /* low-confidence predicate */
    final mapResult = aot.invoke('Iterable.map', <dynamic>[]);
    final growableList = _GrowableList._of(mapResult);
    return growableList;
  } else {
    if (aot.unresolvedRegister('r4') <= 55) {  /* low-confidence predicate */
    } else {
    }
  }
  // Dynamic-call evidence:
...
dynamic e11SyncGen(List<dynamic> args) {
  while (aot.unresolvedRegister('r8') < arg0) {
    if (aot.unresolvedRegister('r6') < arg1) {  /* low-confidence predicate */
    }
    if (aot.unresolvedRegister('r1') != 0) {  /* low-confidence predicate */
      final e11SyncGenResult = e11SyncGen(aot.unresolvedValue('slot 0x10'), aot.unresolvedValue('slot 0x14'));
    } else {
      if (aot.unresolvedRegister('r8') == aot.unresolvedRegister('r0')) {  /* low-confidence predicate */
...
dynamic e10AsyncLoop(List<dynamic> args) async {
  while (aot.unresolvedRegister('r4') < arg0) {
    if (aot.unresolvedRegister('r2') < arg1) {  /* low-confidence predicate */
    }
    final futureResult = aot.invoke('dart:async._Future', <dynamic>[arg1, aot.snapshotRef(18555)]);
    if (aot.unresolvedValue('slot 0x14') == (aot.unresolvedValue('slot 0x14') << 1)) {  /* low-confidence predicate */
      ...
      if (aot.unresolvedRegister('r1') > 0) {  /* low-confidence predicate */
        ...
        // unresolved predicate: r2 > 100 /* low-confidence predicate */
      }
    } else {
    }
  }
...
dynamic e09TryRethrow(List<dynamic> args) {
  ...
  //   'ok:'
  final combinedResult = aot.invoke('String.+', <dynamic>[]);
  if (aot.unresolvedRegister('r2') > 1) {  /* low-confidence predicate */
...
dynamic e06RecordDestructure(List<dynamic> args) {
  while (aot.unresolvedRegister('r0') < 2) {
  }
  // Dynamic-call evidence:
...
dynamic e05NullFlow(List<dynamic> args) {
  ...
  if (aot.unresolvedRegister('r2') != lookupResult) {  /* low-confidence predicate */
    if (aot.unresolvedValue('slot 0x8') != aot.unresolvedRegister('r4')) {  /* low-confidence predicate */
      if (aot.unresolvedRegister('r2') == aot.unresolvedRegister('r0')) {  /* low-confidence predicate */
        if (aot.unresolvedValue('slot 0x8') != aot.unresolvedRegister('r0')) {  /* low-confidence predicate */
          final lookupResult2 = aot.invoke('Map.lookup', <dynamic>[arg0, 'missing']);
          if (aot.unresolvedRegister('r2') != lookupResult2) {  /* low-confidence predicate */
            if (aot.unresolvedRegister('r2') != aot.unresolvedRegister('r0')) {  /* low-confidence predicate */
            } else {
              final updatedItemResult =
                aot.invoke('__Map&_HashVMBase&MapMixin&_HashBase&_OperatorEqualsAndHashCode&_LinkedHashMapMixin.[]=', <dynamic>[arg0, 'missing', aot.unresolvedValue('slot 0x8')]);
            }
          } else {
          }
        } else {
...
dynamic e02Cascade(List<dynamic> args) {
  ...
  if (aot.unresolvedRegister('r1') != aot.unresolvedRegister('r8')) {  /* low-confidence predicate */
    return growableList;
  } else {
    ...
    if ((toStringAsFixedResult & 1) == 0) {  /* low-confidence predicate */
      if ((arg0 & 1) == 0) {  /* low-confidence predicate */
        if (arg2 > 0) {  /* low-confidence predicate */
          ...
          // unresolved predicate: arg1 > 10 /* low-confidence predicate */
        }
      } else {
      }
    } else {
...
class E13Dynamic {
  ...
  dynamic noSuchMethod(List<dynamic> args) {
    ...
    if ((get_memberNameResult & 1) == 0) {  /* low-confidence predicate */
      ...
      if ((aot.unresolvedRegister('r0') & 1) == 0) {  /* low-confidence predicate */
        return 'unhandled:${get_memberNameResult}${aot.snapshotRef(244)}${aot.unresolvedValue('interpolated part')}';
      } else {
      }
    } else {
    }
...
class E15Vec implements Comparable<E15Vec> {
  ...
  dynamic compareTo(List<dynamic> args) {
    if (aot.unresolvedRegister('r4') == 756) {  /* low-confidence predicate */
      if (aot.unresolvedRegister('r8') == aot.unresolvedRegister('r0')) {  /* low-confidence predicate */
        if (aot.unresolvedRegister('r3') == aot.unresolvedRegister('r0')) {  /* low-confidence predicate */
  ...
  dynamic operator ==(List<dynamic> args) {
    if (aot.unresolvedRegister('r2') != 756) {  /* low-confidence predicate */
    } else {
    }
    // 1 branch region(s) and 0 loop(s) reconstructed; exact machine structure remains in reports/functions.json.
...
abstract class E21Mode extends _Enum {
  ...
  num _slot_4; // AOT slot +0x4; unboxed_field
  num _slot_8; // AOT slot +0x8; unboxed_field
  dynamic _slot_c; // AOT slot +0xc; reference
  num _slot_10; // AOT slot +0x10; unboxed_field
